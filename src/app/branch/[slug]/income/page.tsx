...
export default function BranchIncomePage() {
  ...
  const fetchData = () => {
    ...
    fetch(`/api/branch/income?branchId=${session.user.branchId}`)
      .then((res) => res.json())
      .then((data) => { setIncomes(data.incomes); setCategories(data.categories); })
      .catch(console.error)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-black text-slate-900">รายรับ</h1>
          <p className="text-sm text-slate-500 mt-1">บันทึกและดูประวัติรายรับ</p>
        </div>
        <Button icon="add" onClick={() => setShowAdd(true)}>เพิ่มรายรับ</Button>
      </div>

      <div className="rounded-2xl border-l-4 border-l-success bg-success-light p-5">
        <p className="text-[11px] font-bold uppercase tracking-wider text-slate-500">รายรับทั้งหมด</p>
            ...
            <tbody>
              {incomes.map((item) => (
                <tr key={item.id} className="border-b border-slate-50 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3 px-4"><Badge variant="success">{item.category?.name || "-"}</Badge></td>
                  <td className="py-3 px-4 text-slate-600">{item.description || "-"}</td>
                  <td className="py-3 px-4 text-right font-semibold text-emerald-600">+฿{item.amount.toLocaleString()}</td>
                </tr>
              ))}
              {incomes.length === 0 && <tr><td colSpan={4} className="py-8 text-center text-slate-400">ยังไม่มีรายรับ</td></tr>}
            </tbody>
          </table>
        </div>
      </Card>

      <Modal isOpen={showAdd} onClose={() => setShowAdd(false)} title="เพิ่มรายรับ">
